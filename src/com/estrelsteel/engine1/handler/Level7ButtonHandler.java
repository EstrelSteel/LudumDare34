...
public  class Level7ButtonHandler extends Handler {
	
	private Engine1 engine;
	private Tile tile;
	
	public Level7ButtonHandler(String name, Engine1 engine, Tile tile) {
		super(name);
		this.engine = engine;
		this.tile = tile;
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getX() >= displayX && e.getX() <= displayX + tile.getLocation().getWidth() && e.getY() >= displayY && e.getY() <= displayY + tile.getLocation().getHeight()) {
			...
			Tile tile;
			listTile = engine.slvl7.getTiles();
			for(int i = 0; i < listTile.size(); i++) {
				tile = listTile.get(i);
				if(tile.getType() == TileType.BUTTON_BLUE && engine.lvl7Finished != 2) {
					listTile.remove(i);
					i--;
				}
			}
			engine.slvl7.setEntities(list);
			engine.slvl7.setTiles(listTile);
			engine.world = engine.slvl7;
			engine.player.getLocation().setX(0);
			engine.player.getLocation().setY(0);
			engine.hud.setOpen(true);
			engine.passCount = 50;
			engine.passPercent = 60.0;
			engine.passWithoutStone = true;
			engine.wiltChance = 1000;
			Engine1.twoButtonControl = true;
		}
	}
