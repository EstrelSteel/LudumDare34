...
public  class Level5ButtonHandler extends Handler {
	
	private Engine1 engine;
	private Tile tile;
	
	public Level5ButtonHandler(String name, Engine1 engine, Tile tile) {
		super(name);
		this.engine = engine;
		this.tile = tile;
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getX() >= displayX && e.getX() <= displayX + tile.getLocation().getWidth() && e.getY() >= displayY && e.getY() <= displayY + tile.getLocation().getHeight()) {
			...
			entities = engine.slvl5.getEntities();
			...
			engine.slvl5.setEntities(entities);
			engine.world = engine.slvl5;
