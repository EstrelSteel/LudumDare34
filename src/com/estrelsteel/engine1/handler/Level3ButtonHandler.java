...
public  class Level3ButtonHandler extends Handler {
	
	private Engine1 engine;
	private Tile tile;
	
	public Level3ButtonHandler(String name, Engine1 engine, Tile tile) {
		super(name);
		this.engine = engine;
		this.tile = tile;
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getX() >= displayX && e.getX() <= displayX + tile.getLocation().getWidth() && e.getY() >= displayY && e.getY() <= displayY + tile.getLocation().getHeight()) {
			...
			entities = engine.slvl3.getEntities();
			...
			engine.slvl3.setEntities(entities);
			engine.world = engine.slvl3;
