...
public  class Level2ButtonHandler extends Handler {
	
	private Engine1 engine;
	private Tile tile;
	
	public Level2ButtonHandler(String name, Engine1 engine, Tile tile) {
		super(name);
		this.engine = engine;
		this.tile = tile;
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getX() >= displayX && e.getX() <= displayX + tile.getLocation().getWidth() && e.getY() >= displayY && e.getY() <= displayY + tile.getLocation().getHeight()) {
			...
			entities = engine.slvl2.getEntities();
